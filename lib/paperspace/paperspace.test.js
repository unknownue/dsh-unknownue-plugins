    ...
    try {
      for await (const chunk of this.face.stream({
        ...
        messages: input.messages.map(
          (message) => message.role === "system" ? { role: "system", content: [{ type: "text", text: message.content }], source: { kind: "plugin", plugin: "dsh-unknownue-plugins" } } : { role: "user", content: [{ type: "text", text: message.content }], source: { kind: "user" } }
        ),
...
function buildProvider(ctx) {
  ...
  return new OpenAICompatibleProvider({ baseUrl: ctx.provider.baseUrl, apiKey: ctx.provider.apiKey ?? void 0, model: ctx.provider.model, timeoutMs: ctx.timeoutMs });
}
