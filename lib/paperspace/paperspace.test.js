...
var translateModelSchema = z.object({ provider: z.string().min(1).max(200), model: z.string().min(1).max(200) }).strict();
...
function applySettingsInput(input, current, row) {
  ...
  return {
    ...
    translateModel: input.translateModel !== void 0 ? input.translateModel : base.translateModel,
...
var LISTING_LINK_RE = /^\[⬇\]\(data:text\/[^)]*\)$/;
var LISTING_CAPTION_RE = /^(?:Algorithm|Table|Figure|Alg\.?|Listing|Pseudocode)\s/i;
...
// src/host/paperspace/proxy.ts
import { ProxyAgent } from "undici";
function resolveProxyUrl(configProxy) {
...
function createProxyFetchFn(proxyUrl) {
...
function rewriteImageUrls(markdown, urlMap, baseUrl) {
  ...
  return markdown.replace(MD_IMG_RE, (full, url) => rewrite(full, url)).replace(HTML_IMG_RE, (full, url) => rewrite(full, url));
}
...
var LISTING_PREFIX = "\uE000l";
var LISTING_RE = /\uE000l(\d+)\uE001/g;
...
function htmlToMarkdown(html) {
  ...
  const listingMarkdown = /* @__PURE__ */ new Map();
  let listingIndex = 0;
  const listingPlaceholder = (id) => LISTING_PREFIX + id + "\uE001";
  const buildListingMarkdown = (captionLines, $listing) => {
    let code = "";
    ...
    if (!code) {
      code = $listing.find(".ltx_listingline").map((_j, el) => $(el).text()).get().join("\n");
    }
  ...
  $("figure.ltx_float").each((_index, fig) => {
    ...
    const md = $listings.map((_j, el) => buildListingMarkdown(captionLines, $(el))).get().join("\n\n");
  ...
  const restoreMath = (text) => text.replace(PLACEHOLDER_RE, (_match, id) => {
...
var DshLlmProvider = class {
  constructor(face, provider, model, timeoutMs) {
    this.face = face;
    ...
    try {
      const iterable = this.face.stream({
        ...
        messages: input.messages.map(
          (message) => message.role === "system" ? { role: "system", content: [{ type: "text", text: message.content }] } : { role: "user", content: [{ type: "text", text: message.content }] }
        ),
...
function buildProvider(ctx) {
  ...
  return new OpenAICompatibleProvider({ baseUrl: ctx.provider.baseUrl, apiKey: ctx.provider.apiKey ?? void 0, model: ctx.provider.model, timeoutMs: ctx.timeoutMs, proxyUrl: ctx.proxyUrl });
}
...
function protectMath(text) {
  ...
  for (const { match, marker } of replacers) {
    inProgress = inProgress.replace(match, (span) => {
  ...
  return {
    ...
    restore(translated) {
      return translated.replace(RESTORE_RE, (_whole, inline, block) => {
...
var fakeLlm = {
  ...
  listModels: async (provider) => provider === "mock" ? [{ provider: "mock", id: "mock-1", name: "Mock One" }] : [],
...
async function main() {
  ...
  try {
    ...
    mkdirSync(join2(config.workspaceDir, "papers"), { recursive: true });
    const staleMdFile = join2(config.workspaceDir, "papers", "1706.03762.md");
    ...
    check("workspace paper md removed on delete", !existsSync2(staleMdFile));
    ...
    check(
      ...
      mergedCodeBlocks.length === 2 && mergedCodeBlocks[0].text === "Para before code, no blank line:" && mergedCodeBlocks[1].text === "Para after code, no blank line."
    );
    ...
    const legacyListingMd = "Algorithm 1 ELF: training.\n\n[\u2B07](data:text/plain;base64,xyz)\n\nx \\= encode(s)\n\n# comment line\n\nAlgorithm 2 ELF: inference.\n\n[\u2B07](data:text/plain;base64,abc)\n\ny \\= corrupt(x)\n\nThe core concepts of ELF are summarized in a long prose paragraph that definitely exceeds two hundred and fifty characters so the listing region ends here and the prose gets translated normally, continuing on for a while longer still, with additional sentences appended to guarantee the length check passes reliably for this regression test case.";
    const legacyListingBlocks = splitParagraphs(legacyListingMd);
    check(
      "legacy listing code lines skipped (captions + prose kept)",
      legacyListingBlocks.length === 3 && legacyListingBlocks[0].text.includes("Algorithm 1") && legacyListingBlocks[1].text.includes("Algorithm 2") && legacyListingBlocks[2].text.includes("The core concepts")
    );
    const listingHtml = '<main><section><figure class="ltx_figure"><div class="ltx_flex_figure"><div class="ltx_flex_cell"><figure id="alg1" class="ltx_float ltx_figure_panel ltx_float_algorithm"><figcaption class="ltx_caption"><span class="ltx_tag ltx_tag_float">Algorithm 1</span> Train.<br class="ltx_break"><span>Note line.</span></figcaption><div class="ltx_listing ltx_lst_language_PythonFuncColor ltx_lstlisting"><div class="ltx_listing_data"><a href="data:text/plain;base64,IyBjb21tZW50CnggPSAxCnlbaV0gPSAy" download="">\u2B07</a></div><div class="ltx_listingline"># comment</div><div class="ltx_listingline">x = 1</div></div></figure></div></div></figure><p>After paragraph.</p></section></main>';
    const listingMd = htmlToMarkdown(listingHtml);
    check(
      "html2md converts listings to fenced code (base64 decoded)",
      listingMd.includes("Algorithm 1 Train.") && listingMd.includes("Note line.") && listingMd.includes("```python\n# comment\nx = 1\ny[i] = 2\n```") && !listingMd.includes("data:text/plain;base64") && listingMd.includes("After paragraph.")
    );
    ...
    const objectHtml = '<main><figure class="ltx_figure"><figcaption class="ltx_caption"><span class="ltx_tag ltx_tag_figure">Figure X</span>: A plot.</figcaption><div class="ltx_block ltx_figure_panel"><object type="image/svg+xml" data="2605.10938v2/system_teaser.svg" width="229" height="87"></object></div></figure></main>';
    ...
    const rewrittenFallback = rewriteImageUrls("![a](rel/a.png) ![b](miss/x.png) ![d](data:image/png;base64,xx)", /* @__PURE__ */ new Map([["https://x/base/rel/a.png", "/local/a.png"]]), "https://x/base/");
    check(
      "rewriteImageUrls: unstored relative \u2192 absolute, data: untouched",
      rewrittenFallback.includes("/local/a.png") && rewrittenFallback.includes("https://x/base/miss/x.png") && rewrittenFallback.includes("data:image/png;base64,xx")
    );
    ...
    await runTranslationJob(claimedD, mdD, { translations, provider: dshProvider, llm: fakeLlm, timeoutMs: 1e4, maxAttempts: 3 });
    const snapD = await translations.findSnapshot(paperD.id, "zh-CN");
    check(
      "DSH-route translation e2e (llm stream, reasoning deltas ignored)",
      snapD?.status === "completed" && snapD.paragraphs?.[0] === "[DSH] DSH paragraph one." && snapD.paragraphs?.[1] === "[DSH] DSH paragraph two." && snapD.glossary.transformer === "\u53D8\u6362\u5668"
    );
    ...
    let fatal = null;
    try {
      await runTranslationJob(claimedE, "# T\n\nP1.", { translations, provider: dshProvider, llm: null, timeoutMs: 1e3, maxAttempts: 3 });
    } catch (error) {
      fatal = error;
    }
    check("DSH-route job without llm service \u2192 fatal error", fatal instanceof TranslationFatalError);
    if (fatal) await failTranslationJob(claimedE, fatal, { translations, provider: dshProvider, llm: null, timeoutMs: 1e3, maxAttempts: 3 });
