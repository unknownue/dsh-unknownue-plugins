...
async function removePaperMarkdown(workspaceDir, arxivId) {
...
import { mkdir as mkdir2, rm as rm2, stat, writeFile as writeFile2 } from "node:fs/promises";
...
var FileObjectStore = class {
  ...
  async deleteObject(key) {
    await this.ensureBucket();
    await rm2(this.keyPath(key), { force: true });
  }
...
function disposeProxyAgent() {
  cachedAgent?.agent.close().catch(() => {
  });
  cachedAgent = null;
}
function proxyFetch(url, init = {}, proxyUrl) {
...
async function listDshModelDirectory(llm) {
  const face = llm;
...
var translateInput = z2.object({ target_lang: translateLang }).strict();
...
function toJob(row) {
  return {
    ...
    provider: row.provider ? "provider" in row.provider ? { provider: row.provider.provider, model: row.provider.model } : { baseUrl: row.provider.baseUrl, model: row.provider.model } : null
  };
}
...
function registerRoutes(webServer, host, getLlm) {
  ...
  webServer.register({
    ...
    handler: wrap(async (req, res) => {
      ...
      if (action === "translate-paper" && rest.length === 2) {
        ...
        const persisted = { provider: selection.provider, model: selection.model };
...
function fetchWithTimeout(url, timeoutMs, init = {}, proxyUrl) {
...
async function fetchArxivMetadata(arxivId, timeoutMs, proxyUrl) {
...
async function fetchArxivHtml(arxivId, timeoutMs, proxyUrl) {
  const primary = `https://arxiv.org/html/${arxivId}`;
  const primaryTimeoutMs = Math.min(timeoutMs, 12e3);
...
var ingestAttempts = /* @__PURE__ */ new Map();
  ...
  while (current !== void 0 && current !== null && !seen.has(current)) {
    ...
    if (current instanceof Error) {
      ...
      for (const key of ["code", "severity", "detail", "hint", "position", "routine", "query", "errno", "syscall", "address", "port"]) {
        ...
        if (value !== void 0) extra[key] = value;
      }
...
function startWorker(ctx, runtime, store, config, getLlm) {
  ...
  ctx.effect(() => {
    ...
    const translateTimer = setInterval(() => {
      ...
      const watchdog = setTimeout(() => {
        liveness.lastError = "watchdog: tick overran " + (translateTimeoutMs + 6e4) + "ms";
        console.error("[paperspace] translation watchdog fired \u2014 releasing queue slot");
        translating = false;
      }, translateTimeoutMs + 6e4);
      void translateOne().catch((error) => {
        liveness.lastError = messageOf2(error);
