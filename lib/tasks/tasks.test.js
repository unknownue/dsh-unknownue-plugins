...
var PRESET_TODOS_MAX = 20;
var PRESET_TODO_MAX_LEN = 200;
function normalizePresetTodos(raw) {
...
function loadSettingsFile() {
  try {
    ...
    const file = { version: 1, dataDir: normalizePath(record.dataDir) };
    const presetTodos = normalizePresetTodos(record.presetTodos);
    if (presetTodos !== void 0) file.presetTodos = presetTodos;
    return file;
  } catch {
    return null;
  }
...
async function main() {
  ...
  const todoCard = withTodos.body.card;
  check(
    "create with todos roundtrips + mints ids",
    withTodos.status === 200 && Array.isArray(todoCard.todos) && todoCard.todos.length === 2 && typeof todoCard.todos[0].id === "string" && todoCard.todos[0].content === "\u62C6\u9700\u6C42" && todoCard.todos[0].done === false && todoCard.todos[1].content === "\u5199\u4EE3\u7801" && todoCard.todos[1].done === true,
  ...
  const toggledCard = toggled.body.card;
  check(
  ...
  const keepCard = patchNoTodos.body.card;
  check(
  ...
  check("patch todos: [] clears the list", cleared.body.card.todos.length === 0);
  ...
  const pointTimeCard = pointTime.body.card;
  check(
  ...
  const rangeCard = range.body.card;
  check(
  ...
  const patchDueCard = patchDue.body.card;
  check(
  ...
  const patchKeepCard = patchKeepDue.body.card;
  check(
  ...
  const dueClearedCard = dueCleared.body.card;
  ...
  const pointAgainCard = pointAgain.body.card;
  ...
  const tagCard = withTags.body.card;
  check(
  ...
  const patchTagCard = patchTags.body.card;
  ...
  const patchKeepTagCard = patchKeepTags.body.card;
  ...
  check("patch tags: [] clears list", tagsCleared.body.card.tags.length === 0);
  ...
  const reopenedCard = reopenedTasks.find((t) => t.id === persistedId);
  check(
    "subtasks survive dispose + reopen",
    reopenedCard !== void 0 && Array.isArray(reopenedCard.todos) && reopenedCard.todos.length === 1 && reopenedCard.todos[0].content === "\u8DE8\u91CD\u542F\u5B50\u4EFB\u52A1" && reopenedCard.todos[0].done === true,
    JSON.stringify(reopenedCard?.todos)
  );
  check(
    "due range survives dispose + reopen",
    reopenedCard !== void 0 && reopenedCard.due !== null && reopenedCard.due.kind === "range" && reopenedCard.due.start === "2026-09-10T10:00" && reopenedCard.due.end === "2026-09-11",
    JSON.stringify(reopenedCard?.due)
  );
  check(
    "tags survive dispose + reopen",
    reopenedCard !== void 0 && Array.isArray(reopenedCard.tags) && reopenedCard.tags.length === 1 && reopenedCard.tags[0] === "\u8DE8\u91CD\u542F\u6807\u7B7E",
  ...
  check(
    ...
    presetsOnDisk !== void 0 && presetsOnDisk.length === 2 && presetsOnDisk[0] === "\u9700\u6C42\u786E\u8BA4" && presetsOnDisk[1] === "\u65B9\u6848\u8BBE\u8BA1",
    JSON.stringify(presetsOnDisk)
  );
  const gotPresets = await call(api2, "GET", `${TASKS_API}/settings`);
  const gotPresetList = gotPresets.body.settings.presetTodos;
  ...
  check("preset_todos null resets to built-in defaults", resetPresets.status === 200 && loadSettingsFile()?.presetTodos === void 0, JSON.stringify(loadSettingsFile()?.presetTodos));
