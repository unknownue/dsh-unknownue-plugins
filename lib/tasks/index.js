...
var RANK_STEP = 1024;
var COLUMNS = "id, title, body, status, priority, due_at, rank, archived, created_at, updated_at, completed_at, todos";
...
async function updateCard(runtime, id, patch) {
  ...
  await runtime.query(
    ...
    [
      ...
      patch.dueAt !== void 0 ? patch.dueAt : current.due_at,
      status,
      rank,
      completedAt,
      now,
...
var dueSchema = z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "due_at must be YYYY-MM-DD").nullable();
