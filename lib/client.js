  ...
  factory: (require) => {
    ...
    function HostForm(props) {
      ...
      const onPushKey = async () => {
        ...
        setPushMsg(null);
        try {
          // First save the host temporarily to push key
          const tempHost = {
            id: form.id || "temp",
          ...
          // Use test connection params directly
      ...
      return react.createElement(
        ...
        react.createElement(
          ...
          react.createElement("input", { className: "dmk-workdir", value: form.keyPath, onChange: set("keyPath"), placeholder: "如 ~/.ssh/id_ed25519（留空自动生成）" }),
          react.createElement(
            "div",
            { className: "dmr-key-actions" },
            react.createElement("button", { type: "button", className: "dmr-btn", disabled: keyBusy, onClick: onGenerateKey }, keyBusy ? "生成中..." : "生成密钥"),
            keyInfo && keyInfo.publicKey ? react.createElement("button", { type: "button", className: "dmr-btn", onClick: onCopyPublicKey }, "复制公钥") : null
          ),
          keyMsg ? react.createElement("div", { className: "dmr-meta" + (keyMsg.ok ? "" : " dmr-error") }, keyMsg.text) : null,
          keyInfo && keyInfo.publicKey ? react.createElement(
            "div",
            { className: "dmr-pubkey-box" },
            react.createElement("div", { className: "dmr-pubkey-label" }, "公钥内容（需添加到远程主机的 ~/.ssh/authorized_keys）："),
