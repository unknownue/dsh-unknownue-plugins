...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Default remote config (overridden by host-side config if available)
    let remoteConfig = { hosts: [], tunnel: null };
    ...
    function RemoteControl(props) {
      ...
      const loadStatus = async () => {
        try {
          ...
          const hostList = Object.values(value.hosts || {}).map(h => ({
            id: h.id,
            sshHost: h.sshHost,
            label: h.label || h.id,
          ...
          setTunnelStatus(value.tunnel || null);
          if (hostList.length > 0 && !selectedHost) {
            setSelectedHost(hostList[0].id);
          }
        } catch (err) {
          console.error("dsh-unknownue-plugins: remote status failed:", err);
        }
      };
      ...
      const onUpload = async () => {
        ...
        setTransferMsg(null);
        try {
          const result = await call(REMOTE_API, "upload", {
            hostId: selectedHost,
            localPath: localFile,
            remotePath: uploadPath.trim()
          });
          setTransferMsg({ ok: true, text: `已上传: ${result.uploaded} → ${result.to} (${result.size} bytes)` });
        } catch (err) {
      ...
      const onDownload = async () => {
        ...
        setTransferMsg(null);
        try {
          const homeDir = "/tmp"; // fallback
          const result = await call(REMOTE_API, "download", {
            hostId: selectedHost,
            remotePath: downloadPath.trim(),
            localPath: homeDir
          });
          setTransferMsg({ ok: true, text: `已下载: ${result.from} → ${result.downloaded} (${result.size} bytes)` });
        } catch (err) {
          setTransferMsg({ ok: false, text: err instanceof Error ? err.message : String(err) });
        } finally {
          setTransferBusy(false);
      ...
      const onTunnelAction = async (action) => {
        setTunnelBusy(true);
        try {
          const result = await call(REMOTE_API, `tunnel.${action}`, {});
          setTunnelStatus(result);
        } catch (err) {
          setTunnelStatus({ error: err instanceof Error ? err.message : String(err) });
        } finally {
          setTunnelBusy(false);
        }
      };
      ...
      // Build host selector
      const hostSelector = hosts.length > 0
        ? react.createElement(
            "div",
            { className: "dmr-hosts" },
            hosts.map(h => react.createElement(
              "button",
              {
                key: h.id,
                type: "button",
              ...
              react.createElement("span", {
                className: "dmr-dot",
                style: { background: h.reachable ? "var(--dsw-alias-state-success)" : "var(--dsw-alias-state-error)" }
              }),
              h.label
            ))
          )
        : react.createElement("div", { className: "dmr-meta" }, "未配置远程主机");

      // Tunnel section
      const tunnelSection = tunnelStatus ? react.createElement(
        "div",
        ...
        react.createElement(
          ...
          react.createElement("span", {
            className: "dmr-dot",
            style: { background: tunnelStatus.portListening ? "var(--dsw-alias-state-success)" : "var(--dsw-alias-state-error)" }
          }),
          react.createElement("span", null, tunnelStatus.portListening ? `端口 ${tunnelStatus.port} 监听中` : `端口 ${tunnelStatus.port} 未监听`),
          tunnelStatus.service ? react.createElement("span", { className: "dmr-meta" },
            ` · 服务: ${tunnelStatus.service.status}${tunnelStatus.service.uptime ? " · " + tunnelStatus.service.uptime : ""}`
          ) : null,
      ...
      return react.createElement(
        ...
        react.createElement(
          ...
          hostSelector,
          // Command execution
          currentHost && currentHost.reachable ? react.createElement(
            "div",
            { className: "dmr-section" },
            ...
            execOutput.length > 0 ? react.createElement(
              ...
              execOutput.map((line, i) => react.createElement(
                "div",
                { key: i, className: "dmr-output-" + line.type },
                line.text
              ))
            ) : null
          ) : null,
          ...
          currentHost && currentHost.reachable ? react.createElement(
            ...
            react.createElement(
              ...
              react.createElement("input", {
                type: "file",
                className: "dmr-file-input",
                onChange: (e) => setLocalFile(e.target.files?.[0]?.path || e.target.value)
              }),
              react.createElement("input", {
                type: "text",
                className: "dmk-workdir",
                value: uploadPath,
                placeholder: "远程路径 (如 /tmp/file.txt)",
            ...
            react.createElement(
              ...
              react.createElement("input", {
                ...
                className: "dmk-workdir",
                value: downloadPath,
                placeholder: "远程文件路径 (如 /tmp/file.txt)",
