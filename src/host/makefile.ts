 ...
 */
export function parseMakefile(content: string): MakefileList {
...
// ── route dispatch ──────────────────────────────────────────────────────────
interface ListTargetsInput {
  workdir: string | undefined;
  makefile: string;
}

async function listTargets({ workdir, makefile }: ListTargetsInput) {
...
export async function makefileDispatch(
  config: { makefile: string },
  method: string,
  params: Record<string, unknown>
): Promise<{ makefile: string; path: string; targets: MakefileTarget[]; defaultTarget: string | null }> {
  switch (method) {
    case "listTargets":
      return listTargets({
